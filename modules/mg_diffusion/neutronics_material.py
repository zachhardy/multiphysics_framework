...
class NeutronicsMaterial(MaterialBase):
    ...
    def __init__(self, material_id=0, Na=1., sig_r=[], sig_t=[], 
        ...
        self.num_grps= len(sig_r)
        
        # Total cross section
        if sig_t != []:
            assert len(sig_t)==self.num_grps, "Invalid group structure."
            self.sig_t = Na * np.atleast_1d(sig_t)

        # Diffusion coefficient
        if D != []:
            assert len(D)==self.num_grps, "Invalid group structure."
        ...
        if sig_s != []:
            ...
            assert sig_s.shape[0]==self.num_grps, "Invalid group structure."
            assert sig_s.shape[0]==sig_s.shape[1], "Invalid group structure."
            self.sig_s = Na * sig_s

        # Fission cross sections
        if nu_sig_f != []:
            assert len(nu_sig_f)==self.num_grps, "Invalid group structure."
            self.nu_sig_f = Na * np.atleast_1d(nu_sig_f)
            # Fission spectrum
            if self.num_grps> 1:
                assert len(chi)==self.num_grps, "Invalid group structure."
        ...
        # Velocity
        if v != []:
            assert len(v)==self.num_grps, "Invalid group structure."
            self.v = np.atleast_1d(v)

        # Source
        if q != []:
            assert len(q)==self.num_grps, "Invalid group structure."
